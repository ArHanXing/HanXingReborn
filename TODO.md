...
---
添加杂项多方块独立机器：
亨特反应釜（16并行）：普通独立机器，大化反UI+EMI （tier2）
克劳尔反应釜（32并行、0.5耗时）：普通独立机器，大化反UI+EMI （tier2）
培养缸：普通独立机器，输出槽2，输入槽4. （tier3）
辐照诱变仓：普通独立机器，输出槽2，输入槽4. （tier3）
超临界聚合仪：普通独立机器，输出槽2，输入槽4. （tier3）
全部使用对应tier的multiblock材质和模型。独立配方。
---
添加又一个新颖的多方块机器：
戴森云系统：也没有一般的配方系统。
