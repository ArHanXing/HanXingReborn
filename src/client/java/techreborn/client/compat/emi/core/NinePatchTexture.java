...
public class NinePatchTexture {
	...
	private void render(VertexConsumer consumer, Matrix4f mat, int z, int x, int y, int w, int h) {
		...
		rect(consumer, mat, z, rightLeft, bottomTop, rightWidth, bottomHeight, rightLeftU, bottomTopV, pieceU2, pieceV2);

		if (!tiling) {
			if (centerWidth > 0) {
				rect(consumer, mat, z, leftWidth + x, y, centerWidth, topHeight, leftRightU, pieceV1, rightLeftU, topBottomV);
				rect(consumer, mat, z, leftWidth + x, bottomTop, centerWidth, bottomHeight, leftRightU, bottomTopV, rightLeftU, pieceV2);
			}
			if (centerHeight > 0) {
				rect(consumer, mat, z, x, topHeight + y, leftWidth, centerHeight, pieceU1, topBottomV, leftRightU, bottomTopV);
				rect(consumer, mat, z, rightLeft, topHeight + y, rightWidth, centerHeight, rightLeftU, topBottomV, pieceU2, bottomTopV);
				if (centerWidth > 0) {
					rect(consumer, mat, z, leftWidth + x, topHeight + y, centerWidth, centerHeight, leftRightU, topBottomV, rightLeftU, bottomTopV);
				}
			}
		} else {
			...
			for (int tileXIndex = 0; tileXIndex < tilesX; tileXIndex++) {
				...
				rect(consumer, mat, z, leftWidth + localTileX + x, y, curTileWidth, topHeight, leftRightU, pieceV1, curTileU2, topBottomV);
				rect(consumer, mat, z, leftWidth + localTileX + x, bottomTop, curTileWidth, bottomHeight, leftRightU, bottomTopV, curTileU2, pieceV2);
			}
			...
			for (int tileYIndex = 0; tileYIndex < tilesY; tileYIndex++) {
				...
				rect(consumer, mat, z, x, topHeight + localTileY + y, leftWidth, curTileHeight, pieceU1, topBottomV, leftRightU, curTileV2);
				rect(consumer, mat, z, rightLeft, topHeight + localTileY + y, rightWidth, curTileHeight, rightLeftU, topBottomV, pieceU2, curTileV2);

				for (int tileXIndex = 0; tileXIndex < tilesX; tileXIndex++) {
					int localTileX = tileXIndex * tileWidth;
					int curTileWidth = Math.min(tileWidth, centerWidth - localTileX);
					float curTileU2 = (float) (leftRight + curTileWidth) / (float) textureWidth;
					rect(consumer, mat, z, leftWidth + localTileX + x, topHeight + localTileY + y, curTileWidth, curTileHeight, leftRightU, topBottomV, curTileU2, curTileV2);
				}
			}
		}
	}
	...
	private static void rect(VertexConsumer consumer, Matrix4f mat, int z, int x0, int y0, int w, int h,
		...
		consumer.vertex(mat, x1, y1, z).texture(u1, v1);
		consumer.vertex(mat, x1, y0, z).texture(u1, v0);
		consumer.vertex(mat, x0, y0, z).texture(u0, v0);
	}
}
