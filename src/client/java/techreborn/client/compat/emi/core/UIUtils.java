...
public class UIUtils {
	...
	public static void cookTime(dev.emi.emi.api.widget.WidgetHolder widgets, int ticks, int x, int y) {
		widgets.addText(cookTimeText(ticks), x, y, 0xFF3F3F3F, false);
	}

	private static net.minecraft.text.OrderedText cookTimeText(int ticks) {
		float secs = ticks / 20f;
		return Text.translatable("gui.extra_mod_integrations_core.cook_time", secs).asOrderedText();
	}
	...
	public static Text metricNumber(int number) {
		...
		return Text.translatable("metric.format." + power, chopped);
	}
	...
	public static void renderFluid(MatrixStack matrices, Sprite[] sprites, int color, int x, int areaY,
		...
		BufferBuilder bufferBuilder = tess.begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_TEXTURE_COLOR);
		...
		for (int i = 0; i < fluidStripCount; i++) {
			buildFluidHorizontalStrip(bufferBuilder, model, sprite, (float) x,
				(float) areaY + areaHeight - FLUID_PATCH_HEIGHT * (i + 1), fluidWidth, FLUID_PATCH_HEIGHT, r, g, b);
		}
	...
	private static void buildFluidHorizontalStrip(BufferBuilder bufferBuilder, Matrix4f model,
		...
		buildFluidPatch(bufferBuilder, model, sprite, x0 + FLUID_PATCH_WIDTH * fluidPatchCount, y0, patchRemainder, height, r, g, b);
	}
	...
	private static void buildFluidPatch(BufferBuilder bufferBuilder, Matrix4f model, Sprite sprite,
		...
		bufferBuilder.vertex(model, x0, y1, 1.0F).texture(uMin, vMax).color(r, g, b, 1.0F);
		bufferBuilder.vertex(model, x1, y1, 1.0F).texture(uMax, vMax).color(r, g, b, 1.0F);
		bufferBuilder.vertex(model, x1, y0, 1.0F).texture(uMax, vMin).color(r, g, b, 1.0F);
		bufferBuilder.vertex(model, x0, y0, 1.0F).texture(uMin, vMin).color(r, g, b, 1.0F);
	}
}
